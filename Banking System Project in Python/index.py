...
def write(master, name, oc, pin):
    ...
    frec.write(
        str(strftime("[%Y-%m-%d] [%H:%M:%S]  ", gmtime())) + "     " + oc + "              " + oc + "\n"
    )
    frec.close()
...
def Cr_Amt(accnt, name):
    ...
    creditwn.title("MASA BankNexus System")
    ...
    e1.pack(side="top")
    b = tk.Button(
        creditwn, text="Credit", relief="raised", command=lambda: crdt_write(creditwn, e1.get(), accnt, name)
    )
    b.pack(side="top")
...
def De_Amt(accnt, name):
    ...
    debitwn.title("MASA BankNexus System")
    ...
    e1.pack(side="top")
    b = tk.Button(
        debitwn, text="Debit", relief="raised", command=lambda: debit_write(debitwn, e1.get(), accnt, name)
    )
    b.pack(side="top")
...
def disp_tr_hist(accnt):
    ...
    disp_wn.title("MASA BankNexus System")
...
def logged_in_menu(accnt, name):
    ...
    rootwn.title("MASA BankNexus System" + name)
...
def log_in(master):
    ...
    loginwn.title("MASA BankNexus System")
    ...
    b1.pack(side="top")
    loginwn.bind(
        "<Return>", lambda x: check_log_in(loginwn, e1.get().strip(), e2.get().strip(), e3.get().strip())
    )
...
def Create():
    ...
    crwn.title("MASA BankNexus System")
    ...
    e3.pack(side="top")
    b = tk.Button(
        crwn, text="Submit", command=lambda: write(crwn, e1.get().strip(), e2.get().strip(), e3.get().strip())
    )
    b.pack(side="top")
...
def Main_Menu():
    ...
    rootwn = tk.Tk()
    rootwn.geometry("1600x500")
    rootwn.title("MASA BankNexus System")
    rootwn.configure(background="yellow")
    fr1 = tk.Frame(rootwn)
    fr1.pack(side="top")
